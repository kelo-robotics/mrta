...
class Configurator:
        ...
        for component_name, component in self._components.items():
            if hasattr(component, 'register_robot'):
                for robot_id in fleet:
                    robot = Robot.create_new(robot_id, capabilities=["navigation", "docking"])
                    component.register_robot(robot)
    ...
    def register_api(self, component_name, **kwargs):
        ...
        if robot_id and component_name == 'robot_proxy':
            api_config['zyre']['zyre_node']['node_name'] = str(robot_id) + '_proxy'
        elif robot_id and component_name == 'robot':
            api_config['zyre']['zyre_node']['node_name'] = str(robot_id)
    ...
    def register_store(self, component_name, **kwargs):
        ...
        if robot_id:
            store_config['db_name'] = store_config['db_name'] + '_' + str(robot_id)
